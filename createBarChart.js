function createBarChart(chartID, barData, initialH, initialW){
	/*a target chartID of where to place the chart and
	data in the form of an array to create the barchart
	takes in intial height and width as well, the boolean
	value will decide whether to append the axis to the first
	svg or to a different svg*/
	...
	*/	
	...
	var height = initialH - margin.top - margin.bottom,
	...
	*/
    ...
    //svgChart is used to make sure the axis is attatched to the correct chart
	var svgChart = d3.select(chartID).select("svg")[0][0];
	...
	var yGuide = d3.select(svgChart).append('g')
		yAxis(yGuide)
	...
	var xGuide = d3.select(svgChart).append('g')
		xAxis(xGuide)
	...
	//boolean used to make sure chartID is not #chart2 so does not affect adpatation
	if (chartID != "#chart2"){
		information = generateBarDots();
		return information;
	}
}
